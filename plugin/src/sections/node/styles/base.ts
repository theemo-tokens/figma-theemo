...
export default abstract class BaseStyleAdapter {
  ...
  read() {
    this.local = this.node[`${this.type}StyleId`] ? figma.getStyleById(this.node[`${this.type}StyleId`]) as BaseStyle : undefined;
  }
  ...
  load() {
    ...
    if (data.from) {
      this.from = figma.getStyleById(data.from) as BaseStyle;
    }

    if (data.to) {
      this.to = figma.getStyleById(data.to) as BaseStyle;
    }
  ...
  delete() {
    this.node.setSharedPluginData(NAMESPACE, this.type, '');
  } 

  needsUnlink() {
    return Boolean(this.to && !this.local);
  }
  ...
  protected abstract updateContextStyle(): void;
  ...
  private getStyleData(style: { id: string, name: string } | undefined) {
    if (style) {
      return {
        id: style.id,
        name: style.name
