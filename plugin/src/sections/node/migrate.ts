...
export const NON_COLOR_STYLES = [StyleTypes.Text, StyleTypes.Effect];
...
export function migrateNonColorReferences(infrastructure: Infrastructure) {
  ...
  container.references.eachWithHandler((handler) => {
    for (const style of NON_COLOR_STYLES) {
